...
if __name__ == "__main__":
    asyncio.run(main())    def __init__(self, db_path: str):
        self.db_path = db_path
    ...
    async def get_progress(self, user_id: int, chapter_id: int) -> QuizProgress:
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute(
                ...
                return QuizProgress(user_id=user_id, subject="", chapter="", current_index=0, score=0, answers=[])
    ...
    async def get_top_scorers_weekly(self, limit: int = 3) -> List[UserScore]:
        async with aiosqlite.connect(self.db_path) as db:
            week_ago = (datetime.now() - timedelta(days=7)).strftime('%Y-%m-%d %H:%M:%S')
            async with db.execute("""
                SELECT u.name, u.username, SUM(up.score) as total_score
...
class QuizService:
    ...
    @staticmethod
    def create_question_text(question: Question, current: int, total: int, score: int) -> str:
        """Format question with beautiful styling"""
...
class ModernQuizBot:
    ...
    async def _check_channel_membership(self, user_id: int) -> bool:
        """Check if user is in mandatory channel"""
        try:
            member = await self.bot.get_chat_member(Config.MANDATORY_CHANNEL, user_id)
            return member.status in ['member', 'administrator', 'creator']
        except:
...
📢 **Mandatory Channel:** {channel}

After joining, click "I've Joined" below!
        """.format(channel=Config.MANDATORY_CHANNEL)
    ...
    async def _ask_profile_confirmation(self, chat_id: int):
        """Ask user to confirm their profile"""
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton("✅ Confirm My Profile", callback_data="confirm_profile"))
...
Please confirm this is correct to continue!
        """.format(name=self.bot.get_chat(chat_id).first_name, 
                  username=self.bot.get_chat(chat_id).username or "NoUsername")
...
🤔 **Need assistance?** 
📞 **Contact our admin directly:** @{admin}
...
🔧 **Issues?** Contact admin above!
        """.format(admin=Config.ADMIN_USERNAME)
        
        await self.bot.send_message(chat_id, text, reply_markup=markup, parse_mode='Markdown')

    async def _text_handler(self, message: Message):
        """Handle text messages for button navigation"""
        text = message.text
    ...
    async def _show_subjects(self, chat_id: int):
        ...
        if not subjects:
            await self.bot.send_message(chat_id, "📭 *No subjects available yet!*", parse_mode='Markdown')
            return
    ...
    async def _show_chapters(self, chat_id: int, subject_id: int, subject_name: str):
        ...
        if not chapters:
            await self.bot.send_message(chat_id, f"📭 *No chapters available for {subject_name}!*", parse_mode='Markdown')
            return
    ...
    async def _start_quiz(self, chat_id: int, user_id: int, chapter_id: int):
        ...
        quiz = await self.db.get_quiz(chapter_id)
        if not quiz:
            await self.bot.send_message(chat_id, "❌ *Quiz not available!*", parse_mode='Markdown')
            return
    ...
    async def _send_question(self, chat_id: int, user_id: int, chapter_id: int, question_index: int):
        ...
        question_text = self.quiz_service.create_question_text(
            question, question_index + 1, len(quiz), progress.score
        )
    ...
    async def _handle_answer(self, call: CallbackQuery):
        ...
        try:
            ...
            # Save first answer (prevent cheating)
            if len(progress.answers) <= question_index:
                progress.answers.append(answer_idx)
...
🎯 *Keep taking quizzes to improve your rank!*
    ...
    async def _show_top_scorers(self, chat_id: int):
        """Show top 3 scorers of the week"""
        top_scorers = await self.db.get_top_scorers_weekly(limit=3)
        
        if not top_scorers:
            await self.bot.send_message(chat_id, "📭 *No scores yet this week! Be the first!*", parse_mode='Markdown')
            return
    ...
    async def _document_handler(self, message: Message):
        """Handle document upload for admin"""
        if message.from_user.id != self.admin_id:
            return
    ...
    async def _callback_handler(self, call: CallbackQuery):
        ...
        try:
            ...
            if data == "check_channel":
                ...
                if in_channel:
                    await self._ask_profile_confirmation(call.message.chat.id)
                else:
                    await self.bot.answer_callback_query(call.id, "❌ Please join the channel first!", show_alert=True)
